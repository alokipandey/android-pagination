...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        observeLiveData()
        if(viewModel.apiResponse.value == null  && isOnline(this)) {viewModel.fetchData(viewModel.getNextPageEndPoint())}
        else{noInternetMsg()}
    }
    ...
    private fun observeLiveData() {
        viewModel.apply {
            apiResponse.observe(this@MainActivity, Observer {
                ...
                if (it==null){ when(viewModel.loadedPages){
                    2-> {noMoreData();viewModel.loadedPages++;return@Observer}
                    else -> {loadedPages++;viewModel.fetchData(viewModel.getNextPageEndPoint());return@Observer}
                }}
                if(!(::rvAdapter.isInitialized) && it.posts.count()>0){loadedPages++;initRecyclerView(rv_home_data, it.posts);setupSpinner(spinner_sort)}
                else if(it.posts.count()>0){loadedPages++;rvAdapter.addMoreData(it.posts)}
                else{loadedPages++;viewModel.fetchData(viewModel.getNextPageEndPoint())}
            })
            ...
            addOnScrollListener(object :RecyclerView.OnScrollListener(){
                ...
                override fun onScrolled(recyclerView: RecyclerView, dx: Int, dy: Int) {
                    ...
                    if(viewModel.isLoading.value != true && lastVisibleItemPosition+itemThreshold >= totalItemCount){
                        if (!isOnline(this@MainActivity)){noInternetMsg();return}
    ...
    private fun noMoreData() {
        if (!::rvAdapter.isInitialized)
            Toast.makeText(this@MainActivity,"No feed available for now, come back later.",Toast.LENGTH_SHORT).show()
    ...
    private fun setupSpinner(sortSpinner: Spinner) {
        val arrayAdapter = ArrayAdapter(this, android.R.layout.simple_spinner_item, getSpinnerItems())
            ...
            onItemSelectedListener = object :AdapterView.OnItemSelectedListener{
                ...
                override fun onItemSelected(parent: AdapterView<*>, view: View, position: Int, id: Long){
                    when(parent.getItemAtPosition(position).toString()){
                        SpinnerItems.NEW.value -> {rvAdapter.sortData(SpinnerItems.NEW) }
                        SpinnerItems.OLD.value -> {rvAdapter.sortData(SpinnerItems.OLD) }
                        SpinnerItems.SHARED_DESC.value -> {rvAdapter.sortData(SpinnerItems.SHARED_DESC) }
                        SpinnerItems.SHARED_ASC.value -> {rvAdapter.sortData(SpinnerItems.SHARED_ASC) }
                        SpinnerItems.LIKE_DESC.value -> { rvAdapter.sortData(SpinnerItems.LIKE_DESC)}
                        SpinnerItems.LIKE_ASC.value -> {rvAdapter.sortData(SpinnerItems.LIKE_ASC) }
                        SpinnerItems.VIEWS_DESC.value -> {rvAdapter.sortData(SpinnerItems.VIEWS_DESC) }
                        SpinnerItems.VIEWS_ASC.value -> {rvAdapter.sortData(SpinnerItems.VIEWS_ASC) }
                    }
                }
            }
        }
    ...
    private fun getSpinnerItems(): ArrayList<String> {
        return ArrayList<String>().apply{
            add(SpinnerItems.NEW.value);add(SpinnerItems.OLD.value);add(SpinnerItems.SHARED_DESC.value);add(SpinnerItems.SHARED_ASC.value);
